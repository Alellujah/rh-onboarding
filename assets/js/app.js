...
var cookie = Cookies.get('first-visit');
var cookie='false';
if (cookie == 'true') {
  //já visitou uma vez, não faz onboard
  //Alterado para o nginx
  onylShowHome();
} else {
  //primeira visita
  onboard();
...
function onboard(){
  ...
  //fourth and final
  var element = document.getElementById('thirdpage');
  var hammertime = Hammer(element).on("swipeleft", function(event) {
      $("#thirdpage").fadeOut();
      window.location.href = 'http://welcome.iten.pt/homepage';
  });

  //flag the cookie - remove in prod
  Cookies.set('first-visit', 'true', { expires: 365 });

};

function onylShowHome(){
	$("#homepage").show();
}
...
function checklist(){
  ...
  $("#tasks li").on("click", function(event){
    ...
    if ( $(this).hasClass("checked") ) {
      ...
      $(this).addClass("id" + uniqueId);
      Cookies.set('checked', 'uniqueId');
    }

  });
}
//save the checklist - in progress
...
//Tabs
// function menuTabs() {
//   $(".bottom-menu a").on("click", function(event) {
